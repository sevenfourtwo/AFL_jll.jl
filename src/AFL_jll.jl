...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import AFL_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "AFL_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"AFL", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "AFL_jll", "src", "AFL_jll.jl"))
        @info("AFL_ll dev'ed out to /var/home/joost/.julia/dev/AFL_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("97a75533-01f8-5f0b-a19e-1dd09d2fab95"))
platforms = [Pkg.Artifacts.unpack_platform(e, "AFL", artifacts_toml) for e in artifacts["AFL"]]
...
if best_platform === nothing
    @debug("Unable to load AFL; unsupported platform $(triplet(platform_key_abi()))")
else
